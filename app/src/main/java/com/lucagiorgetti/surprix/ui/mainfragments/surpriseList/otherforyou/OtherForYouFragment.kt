...
class OtherForYouFragment : BaseFragment() {
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        val mViewModel = ViewModelProvider(this)[OtherForYouViewModel::class.java]
        ...
        mViewModel.getOtherForYou(ownerUsername).observe(viewLifecycleOwner) { surprises: List<Surprise> ->
            if (surprises.isNotEmpty()) {
                ...
                fab.setOnClickListener { sendEmailToUser(ownerEmail, surprises) }
            } else {
    ...
    private fun sendEmailToUser(ownerEmail: String, surprises: List<Surprise>) {
        val currentUser = SurprixApplication.instance.currentUser
        val subject = SurprixApplication.instance.getString(R.string.mail_subject, currentUser?.username)
        ...
        val body: Spanned = Html.fromHtml(html, Html.FROM_HTML_MODE_LEGACY)
