...
class SetDetailFragment : BaseSetDetailFragment() {
    var missingListDao = MissingListDao(getInstance().currentUser?.username)
    var doubleListDao = DoubleListDao(getInstance().currentUser?.username)
    var collectionDao = CollectionDao(getInstance().currentUser?.username)
    var navigationMode: CatalogNavigationMode? = null
    var setId: String? = null
    override fun setupView() {
        val setDetailViewModel = ViewModelProvider(this).get(SetDetailViewModel::class.java)
        if (arguments != null) {
            ...
            when (navigationMode) {
                CatalogNavigationMode.CATALOG -> {
                    ...
                    mAdapter = CatalogSetDetailRecyclerAdapter(object : CatalogSetDetailClickListener {
                        override fun onSurpriseAddedToDoubles(s: Surprise) {
                            doubleListDao.addDouble(s.id)
                            Snackbar.make(view!!, getInstance().getString(R.string.add_to_doubles) + ": " + s.description, Snackbar.LENGTH_LONG)
                                    .setAction(getInstance().getString(R.string.discard_btn)) { view: View? -> doubleListDao.removeDouble(s.id) }.show()
                        }

                        override fun onSurpriseAddedToMissings(s: Surprise) {
                            collectionDao.isSetInCollection(finalSetId, object : CallbackInterface<Boolean> {
        ...
        setDetailViewModel.getSurprises(setId, navigationMode).observe(viewLifecycleOwner) { surprises: List<CollectionSurprise> ->
    ...
    private fun showAlertAddSetInCollection(setName: String, surprise: Surprise) {
        ...
        alertDialog.setMessage(String.format(getString(R.string.add_in_collection_message), surprise.description, setName))
        alertDialog.setButton(AlertDialog.BUTTON_POSITIVE, getString(R.string.add_btn)
    ...
    private fun addSurpriseToMissingList(surprise: Surprise) {
        ...
        Snackbar.make(requireView(), getInstance().getString(R.string.added_to_missings) + ": " + surprise.description, Snackbar.LENGTH_LONG)
                .setAction(getInstance().getString(R.string.discard_btn)) { view: View? -> missingListDao.removeMissing(surprise.id) }.show()
    }
}